...
INSERT INTO `gogfake_ofertas`
  (`jogo_id`, `slug`, `plataforma`, `preco_original`, `preco_atual`, `desconto`, `url`, `loja`)
SELECT
  `j`.`id`, `j`.`slug`, 'PC',
  ROUND(49.90 + MOD(`j`.`id` * 37, 251), 2),
  ROUND((49.90 + MOD(`j`.`id` * 37, 251)) * (CASE WHEN MOD(`j`.`id`, 4) = 1 THEN 0.65 WHEN MOD(`j`.`id`, 4) = 2 THEN 0.90 ELSE 1.00 END), 2),
  ROUND(((49.90 + MOD(`j`.`id` * 37, 251)) - ROUND((49.90 + MOD(`j`.`id` * 37, 251)) * (CASE WHEN MOD(`j`.`id`, 4) = 1 THEN 0.65 WHEN MOD(`j`.`id`, 4) = 2 THEN 0.90 ELSE 1.00 END), 2)) / (49.90 + MOD(`j`.`id` * 37, 251)) * 100, 2),
  CONCAT('gogfake/jogo.php?slug=', `j`.`slug`), 'GOGFake'
FROM `tcc`.`jogos` AS `j`

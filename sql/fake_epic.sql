...
--
-- Host: 127.0.0.1
-- Tempo de geração: 28/08/2026 às 14:29
...
INSERT INTO `epicfake_ofertas`
  (`jogo_id`, `slug`, `plataforma`, `preco_original`, `preco_atual`, `desconto`, `url`, `loja`)
SELECT
  `j`.`id`, `j`.`slug`, 'PC',
  ROUND(49.90 + MOD(`j`.`id` * 37, 251), 2),
  ROUND((49.90 + MOD(`j`.`id` * 37, 251)) * (CASE WHEN MOD(`j`.`id`, 4) = 2 THEN 0.60 WHEN MOD(`j`.`id`, 4) = 3 THEN 0.80 ELSE 1.00 END), 2),
  ROUND(((49.90 + MOD(`j`.`id` * 37, 251)) - ROUND((49.90 + MOD(`j`.`id` * 37, 251)) * (CASE WHEN MOD(`j`.`id`, 4) = 2 THEN 0.60 WHEN MOD(`j`.`id`, 4) = 3 THEN 0.80 ELSE 1.00 END), 2)) / (49.90 + MOD(`j`.`id` * 37, 251)) * 100, 2),
  CONCAT('epicfake/jogo.php?slug=', `j`.`slug`), 'EpicFake'
FROM `tcc`.`jogos` AS `j`
